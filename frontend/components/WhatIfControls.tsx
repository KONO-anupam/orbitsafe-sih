...
import { MISSION_PROFILE_LABELS, MissionProfileKey } from "@/lib/missionProfiles";
...
export default function WhatIfControls({
  ...
  missionProfile: MissionProfileKey;
  setMissionProfile: (v: MissionProfileKey) => void;
  matchCount: number;
  isDirty: boolean;
  onRunScreening: () => void;
  loading: boolean;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex border rounded-md overflow-hidden" style={{ borderColor: "var(--border)" }}>
          {(Object.keys(MISSION_PROFILE_LABELS) as MissionProfileKey[]).map((key) => (
            <button
              key={key}
              onClick={() => setMissionProfile(key)}
              className="px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.08em] transition-colors"
              style={{
                color: missionProfile === key ? "var(--bg)" : "var(--text-tertiary)",
                background: missionProfile === key ? "var(--accent)" : "transparent",
              }}
              aria-pressed={missionProfile === key}
            >
              {MISSION_PROFILE_LABELS[key]}
            </button>
          ))}
        </div>
      </div>
